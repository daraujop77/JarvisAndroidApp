            ...
            SettingsCard("APP UPDATE") {
                ...
                when (val update = appUpdate) {
                    ...
                    is AppUpdateState.UpToDate -> {
                        Text("JARVIS is up to date (${update.versionName}).", style = MaterialTheme.typography.bodySmall)
                        Spacer(Modifier.height(10.dp))
                        OutlinedButton(onClick = vm::checkForAppUpdate) { Text("Check again") }
                    }
                    is AppUpdateState.Available -> {
                        Text(
                            "Update available: ${update.manifest.version_name} (${update.manifest.version_code})",
                            style = MaterialTheme.typography.bodySmall,
                        )
                        Spacer(Modifier.height(10.dp))
                    ...
                    is AppUpdateState.PermissionRequired -> {
                        ...
                        Spacer(Modifier.height(10.dp))
                        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            Button(onClick = vm::requestAppUpdateInstallPermission) { Text("Allow installs") }
                            OutlinedButton(onClick = vm::installDownloadedAppUpdate) { Text("Continue") }
                        }
                    }
